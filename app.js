...
let unsubs = [];
let currentGuests = []; // Pomocné pole pro lokální filtraci
...
function initApp(uid) {
    ...
    // 2. HOSTÉ (Propisování do všech sekcí + Statistiky)
    unsubs.push(onSnapshot(query(guestsColl, where("userId", "==", uid)), snap => {
        ...
        snap.forEach(d => {
            ...
            const tr = document.createElement('tr'); tr.className = colorClass;
            tr.innerHTML = `
                <td><strong>${g.name}</strong></td><td>${g.city || '-'}</td><td>${g.side}</td>
            ...
            if (g.isHelper) {
                ...
                trH.innerHTML = `<td><strong>${g.name}</strong></td><td><input type="text" class="editable-input" placeholder="Co bude dělat?" value="${g.helperTask || ''}" onchange="updateDoc(doc(db, 'hoste', '${g.id}'), {helperTask: this.value})"></td>`;
            ...
            if (g.needsAcc) {
                ...
                trA.innerHTML = `
                    ...
                    <td><input type="text" class="editable-input" placeholder="Název hotelu/místa" value="${g.accPlace || ''}" onchange="updateDoc(doc(db, 'hoste', '${g.id}'), {accPlace: this.value})"></td>
                    <td><input type="text" class="editable-input" placeholder="Číslo pokoje" value="${g.accRoom || ''}" onchange="updateDoc(doc(db, 'hoste', '${g.id}'), {accRoom: this.value})"></td>
                `;
    ...
    // 3. ROZPOČET (Zůstává nezměněn z minula)
...
window.filterGuests = () => {
    ...
    for (let row of rows) {
        ...
        if (textN.includes(n) && textC.includes(c) && textSi.includes(si) && textSt.includes(st)) {
            row.style.display = "";
        } else {
...
window.filterHelpers = () => {
    ...
    for (let row of rows) {
        let textN = row.cells[0].innerText.toLowerCase();
        let textT = row.cells[1].getElementsByTagName('input')[0].value.toLowerCase(); // čteme hodnotu z inputu
        if (textN.includes(n) && textT.includes(t)) row.style.display = "";
        else row.style.display = "none";
    }
};

// --- AKCE TLAČÍTEK A MODALU ---
...
// Globální funkce (aby fungovaly v HTML onclick)
